...
export function ArticleCard({ article, onClick, index = 0 }: ArticleCardProps) {
  ...
  return (
    <div
      ...
      className={`group relative rounded-3xl bg-slate-950/85 backdrop-blur-2xl ${accent.border} transition-all duration-500 overflow-hidden flex flex-col justify-between futuristic-scroll-card ${
    ...
    >
      ...
      <Link href={`/articles/${article.slug}`} className="block flex-1 flex flex-col justify-between">
        ...
        <div className="px-5 py-3.5 bg-slate-900/90 border-t border-slate-800/80 flex items-center justify-between group-hover:bg-emerald-950/40 transition-colors mt-2 font-mono">
          <span className="text-[11px] font-bold text-emerald-300 uppercase tracking-widest flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
            Ringkasan Artikel
          </span>

          <div className="flex items-center gap-1.5 text-xs font-extrabold text-cyan-300 group-hover:text-cyan-200 group-hover:translate-x-1 transition-transform">
            <span>BACA DETAIL</span>
            <Icon name="arrowRight" className="w-4 h-4 text-cyan-300" />
          </div>
        </div>
      </Link>
    </div>
