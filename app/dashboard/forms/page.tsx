...
import {
  ...
  updateFormStatus,
  deleteForm,
  type FormData as FormRecord,
} from '@/lib/repositories/forms.repo'
import { useAuth } from '@/context/AuthContext'
import { queryKeys } from '@/lib/query-keys'
import { useToast } from '@/lib/hooks/use-toast'
import { filterForms, buildDuplicateTitle, buildDuplicateCode, type FormStatusFilter, type FormViewMode } from './forms-utils'
...
export default function FormsPage() {
  ...
  const formsQuery = useQuery<{ forms: FormRecord[] }>({
  ...
  const reloadForms = () => formsQuery.refetch()
  ...
  // Duplicate / Copy Form (Salin Formulir)
  const handleDuplicateForm = async (form: FormRecord) => {
    if (!form.id) return
  ...
  const handleDeleteForm = async (formId?: string, title?: string) => {
    if (!formId) return
    if (!confirm(`Hapus formulir "${title || formId}" secara permanen?`)) return
    ...
    <div className="flex flex-col min-h-screen bg-[#070913] text-slate-100 font-sans">
      ...
      <div className="flex-1 p-4 sm:p-6 space-y-6 max-w-7xl mx-auto w-full">
        ...
        <div className="rounded-3xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-indigo-500/20 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
          <div className="space-y-1 max-w-2xl">
            ...
            <h2 className="text-lg font-bold text-slate-100">Kuesioner & Instrumen Evaluasi</h2>
            <p className="text-xs text-slate-400">
              Formulir di halaman ini tersimpan dalam skema . Lengkap dengan fitur <strong>Pratinjau Kuesioner</strong> dan <strong>Duplikasi Form</strong>.
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-20 text-slate-400 text-xs gap-3">
            ...
            <span>Memuat daftar kuesioner  dari Firestore...</span>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-20 bg-slate-900/60 border border-slate-800 rounded-3xl space-y-3">
            ...
            <p className="text-base font-bold text-slate-200">Tidak Ada Formulir Ditemukan</p>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              Belum ada formulir yang tersimpan atau tidak ada kuesioner yang cocok dengan filter Anda.
            </p>
            <Link
              href="/dashboard/form-builder"
          ...
          /* CARD GRID VIEW (Distinct  Design) */
      ...
      {previewForm && (
        <PreviewModal
          ...
          formTitle={previewForm.title || 'Pratinjau Formulir'}
