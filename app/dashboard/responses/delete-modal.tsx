...
export default function DeleteModal({ response, isDeleting, onClose, onConfirm }: DeleteModalProps) {
  return (
      ...
      <div className="w-full max-w-md bg-slate-950 border border-rose-500/40 rounded-3xl p-6 shadow-2xl space-y-4">
        <div className="flex items-center gap-3 text-rose-400">
          ...
          <div>
            ...
            <p className="text-xs text-rose-300 font-mono">ID: {response.responseId}</p>
          </div>
        </div>

        <p className="text-xs text-slate-300 leading-relaxed">
          Apakah Anda yakin ingin menghapus data tanggapan kuesioner dari <strong>{(response as any).respondentName || response.answers?.name || 'Responden'}</strong>? Tindakan ini permanen dan tidak dapat dibatalkan.
        </p>
