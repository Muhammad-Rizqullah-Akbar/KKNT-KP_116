...
export type Question = { questionId: string; type: QuestionType; prompt: string; required: boolean; options: QuestionOption[]; presentation: QuestionPresentation; scoring: QuestionScoring; answerKey: AnswerKey }
...
export type FormMetadata = { title: string; description?: string; target?: string; category?: string; kind: FormKind; status: FormStatus }
...
export type PublicQuestion = Omit<Question, 'answerKey' | 'scoring'>
export type PublicFormVersion = Omit<FormVersion, 'questions' | 'scoring'> & { questions: PublicQuestion[] }
